...
Plugins run in their own containers. `core/bin/atlasctl -root . plugin-install DIR` reads the Plugin's `atlas-plugin.json` manifest, issues its integration credential and writes its container configuration under `state/setup/plugins/`; `start` and `stop` then include every installed Plugin, and `plugin-start`, `plugin-stop`, `plugin-restart` and `plugin-force-stop` take a Plugin ID. A planned stop closes admission, quiesces the Plugin and waits up to 15 seconds for finite work; a Plugin that does not cooperate is faulted until `plugin-force-stop`, which records its unfinished attempts as interrupted. `stop` refuses to stop Core while a Plugin cannot stop cooperatively. Clients can cancel an attempt; canceling never stops the Plugin, and a rerun is always a new submission. Clients discover Plugins and submit durable Operation attempts through the SDK. Core retains an attempt before dispatching it, never reruns one, and records `interrupted` when it cannot establish an outcome. The [Plugin container contract](protocol/plugin-container.md) describes what a Plugin implements; [Elevation Lookup](plugins/elevation/README.md) is the example.
