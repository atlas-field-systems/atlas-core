...
The shared number identifies the release those artifacts belong to. Runtime version equality is not required: [compatible client versions are allowed](0005-allow-compatible-client-versions.md). The supported upgrade overlap remains to be designed. This decision does not yet assign the Command Interface, field software, or independently developed Plugins to the shared version sequence, and it does not settle module or subsystem boundaries.
