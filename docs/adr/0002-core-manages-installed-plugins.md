...
Accepted Plugin Operations have a Core-owned attempt identifier and queryable state/outcome within the current run. The caller can request cancellation, but caller disconnection does not cancel accepted work. Request-bound invocation and the old universal 25-second timeout do not define this lifecycle. Core restart wipes Operation records; there is no cross-run recovery requirement.
