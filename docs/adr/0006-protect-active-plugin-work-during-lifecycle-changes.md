...
Configuration startup-failure recovery is awaiting a choice between explicit local restoration/restart and a single automatic restoration/restart. Until resolved, do not interpret either as an accepted implementation rule. Preserve the failed candidate and last working settings for diagnosis; never rerun failed Operations as part of configuration recovery. Schema format, startup success criteria and detailed result fields remain open.
