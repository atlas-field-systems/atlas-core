...
Task records survive ordinary Stop/Start and Restart and are cleared by [Reset](0015-separate-start-stop-restart-and-reset.md). The disconnected-Asset workflow applies while Core remains running. Whole-Core restart handling of active execution is separate from retaining its records and still needs an explicit contract; do not invent automatic reexecution of Tasks.
