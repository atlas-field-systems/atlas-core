...
The reporting contract must prevent an obsolete Asset process from rewriting reconciled state. Exact process/report identity, proof of the current writer and recovery messaging remain engineering work; a status value or newly claimed process ID alone is not sufficient. This does not reinstate the old public execution-session API, require transparent mission continuation across Core restart, or authorize an operator-forced terminal outcome. No new recovery endpoint or Task status is selected here.
