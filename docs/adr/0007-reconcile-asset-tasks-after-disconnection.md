...
The Asset OS owns execution, interruption and the confirmed onboard queue. Core assigns immutable per-Asset submission sequence and records operator-requested order separately from Asset-confirmed order. Assets execute queued Tasks sequentially, oldest submission first by default, following confirmed reordering. Immediate Tasks bypass that queue under the scheduling contract below. Eligible, unstarted queued Tasks, including acknowledged Tasks, may be reordered; started, paused, cancellation-requested and terminal Tasks may not. A disconnected Asset can continue its last confirmed order. Core does not start Tasks or enforce a live readiness gate. Remove the inherited 60-second immediate-start deadline and server-controlled delivery scheduling. Core still validates Command/target contracts and [assigned-Asset report authority](../architecture/system-design.md#identity-and-access); connection loss alone does not prevent tasking. These checks do not require runtime registration or server scheduling.
...
Immediate means prompt Asset handling once received, not guaranteed zero latency or guaranteed delivery during disconnection. The older universal 60-second start deadline and Core-controlled release of the next immediate Task remain excluded. Command-specific validity rules and optional execution deadlines are accepted. The Asset checks validity before acting and reports the outcome; Core does not schedule starts or automatically infer an execution outcome from elapsed time. No universal deadline is selected. Commands must declare their validity behavior; exact deadline fields, clock assumptions and uncertainty handling remain schema work. A late request must not silently bypass its validity rule. Arbitrary immediate-Command conflicts remain Command-specific; Pause/Resume ordering is defined below.
...
No timeout, reconnect, new queued Task, unrelated immediate action or caller status edit implicitly resumes execution. Requesting Resume alone does not clear a reported pause. Asset reports may arrive separately; Task/Asset reads preserve that distinction rather than manufacture atomic confirmation. Newer Pause/Resume intent wins under the control-ordering rule below. Exact report fields and stale-report validation remain implementation work.

## Control ordering and expiry

Accepted after the latest design review: an older Pause or Resume cannot undo newer control intent the Asset has already applied. Compare immutable Core acceptance order for these conflicting control Tasks rather than packet arrival order or client wall clocks. Reuse the existing per-Asset submission sequence where possible; the comparison applies to Pause/Resume intent, not every unrelated immediate action. The Asset must not claim knowledge of a newer request it has not received.

The Asset records older superseded requests explicitly without applying their effects or silently dropping their Task records. A never-executed expired or superseded request reports a non-success outcome with a reason under the existing failure contract; no new Task status is added. An already-terminal Task stays terminal. A previously applied Pause remains a historical successful action even after a later Resume changes the Asset's state. Delayed reports cannot roll operational state back over a newer applied control action. Cancellation-requested Tasks retain cancellation intent until the assigned Asset reports a valid terminal outcome.

An expired newer control must not trigger fallback execution of an older superseded control. Exact interaction of expiry, validated report order and physical actions already in progress must be covered by the Command contract. Core supplies authoritative request order and stores reports; it does not withhold or release execution. Duplicate deliveries and ordinary reordering must pass the integration scenarios before this contract is implemented.

## Recovery after an unexpected Asset restart

Planned restarts during unfinished work remain outside the operating model. After an unexpected Asset-process restart, retained outstanding Tasks are reconciliation input, not instructions to execute every record again. The Asset first establishes which work was completed, still running, suspended or never started, using trustworthy execution evidence. It reports established outcomes and resumes only work whose execution identity/state is reconciled and whose Command permits the action.

If the Asset cannot establish what happened, hold the uncertain work and its queued continuation for explicit recovery. Preserve Core's last known Task state and expose the uncertainty; do not infer success, failure, cancellation or a fresh execution from restart or connectivity alone. A normal reconnect of the same running Asset is distinct from losing execution knowledge in a process crash.

The reporting contract must prevent an obsolete Asset process from rewriting reconciled state. On 23 September 2026 the user accepted a private reporting-authority mechanism as a prerequisite for implementing this recovery behavior. Use a Core-issued process generation and stable report identities with ordering appropriate to the affected state, persisted atomically with accepted effects. Establish replacement-process authority through authenticated reconciliation; an obsolete process must not reclaim authority merely by sending another hello. Core-side report rejection does not prove an old process has physically stopped executing.

Do not discard an unrecorded Task outcome solely because a later telemetry report arrived first. Observation ordering, report deduplication and proof of fresh contact serve different purposes. Exact authority-transfer proof, stream/sequence fields, freshness checks and recovery messages remain engineering work; a status value or newly claimed process ID alone is not sufficient. This does not reinstate the old public execution-session API, require transparent mission continuation across Core restart, or authorize an operator-forced terminal outcome. No new recovery endpoint or Task status is selected here.

## Completed Task retention

Completed and other terminal Tasks remain execution records until Dataset Reset. Operators cannot delete them or rewrite their outcome. Interfaces may show only current work by default; omitting past Tasks from a view does not erase them from Core. Core protects required-result Objects from the time it accepts the assigned Asset's declaration until Reset, regardless of Task status, under the [Object retention contract](0009-expose-objects-only-when-ready.md#required-result-protection). Optional attachments follow ordinary Object deletion rules.
