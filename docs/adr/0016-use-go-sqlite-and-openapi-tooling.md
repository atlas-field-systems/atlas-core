...
Pin tool versions, the supported OpenAPI version and the SQLite driver during the first implementation slice. Prove a nullable update, Task transition, binary upload and change event without patched generator output. Generation does not implement synchronization, safe Object publication or upload retry behavior by itself. On 23 September 2026 the user reaffirmed TypeScript as sufficient for the initial SDK; another SDK language is not an initial prerequisite and mandatory SDK use remains. Transfer/streaming mechanisms remain open. This decision selects a stack; no implementation or benchmark result is claimed.
