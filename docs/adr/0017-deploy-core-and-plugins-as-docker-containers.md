...
Core's Plugins module owns lifecycle policy, including admission, protected stopping and reported outcomes. On 26 September 2026 the user selected a host-side local management module, shared by the CLI and TUI, to carry out lifecycle and Docker actions under that policy. Docker engine control stays on the host, outside Core and Plugin containers, and is not exposed through the public Atlas API or SDK. Do not mount the Docker socket into Core. The private coordination channel remains implementation work. [Docker daemon access](https://docs.docker.com/engine/security/) grants substantial host control.

The host module owns lifecycle-action serialization and interrupted-action recovery across Core shutdown, following [local lifecycle coordination](../architecture/system-design.md#local-lifecycle-coordination). Keeping execution on the host lets Hard Reset complete after Core exits and lets both local callers share one implementation. Core retains domain policy and private data access; the host does not gain a second implementation of Plugin admission or Operation transitions. While Core is stopped, management may perform the setup and lifecycle actions already permitted by [ADR-0015](0015-separate-start-stop-restart-and-reset.md), but cannot accept Plugin Operations.
...
Independent container management does not mean Plugins run without Core. Host-side management enforces the [Core and Plugin runtime lifetime](0015-separate-start-stop-restart-and-reset.md#core-and-plugin-runtime-lifetime), including stopping managed Plugins when Core is spun down. Enforcement after unexpected Core loss must not depend on an operator keeping a CLI command or TUI open. Supervision, shutdown detection and private coordination remain engineering choices; the host placement does not itself require a new always-running management process.
