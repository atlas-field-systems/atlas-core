...
| `DELETE /objects/{object_id}` | Interfaces remove an unprotected Object | Object ID → no body, or conflict for a required result of a completed Task | Validate protection atomically with deletion; arrange stored-content removal only when allowed | Adapt: protect completed-Task required results until Reset |
...
Required result Objects referenced by a completed Task cannot be deleted until Reset. Core checks authoritative Task result references, not only editable Object associations. Metadata edits, Entity deletion and a stale cleanup request cannot bypass protection. Completion and deletion must serialize their readiness/protection decisions. Optional attachments and other unprotected Objects retain ordinary deletion behavior. See [required results](adr/0009-expose-objects-only-when-ready.md#required-results-of-completed-tasks).
