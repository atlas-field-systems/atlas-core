...
| `PUT /entities/{entity_id}/task-order` | Tasking clients reorder an Asset's unstarted Tasks | Complete eligible Task ID list, expected queue revision, request identity → accepted requested revision/order | Reject stale/conflicting edits; publish requested order without implying Asset adoption | New |
...
Task creation assigns one Protocol-defined Command to one Asset and begins in `pending`. Core assigns a permanent increasing submission sequence within that Asset's queue when accepting it. Assignment, Command, and input are immutable. The specified lifecycle is `pending`, `acknowledged`, `in_progress`, `cancellation_requested`, `completed`, `failed`, or `cancelled`. Paused Tasks are a planned extension whose transitions remain to be decided. Terminal Tasks are permanent execution records, so there is no generic Task patch or delete endpoint.
...
3. Specify Task status and queue wire fields, report validation and reconciliation mechanics; resolve pause/resume and immediate execution before adding their transitions. Do not reintroduce the removed execution-session API.
