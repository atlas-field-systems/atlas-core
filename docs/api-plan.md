...
Required result Objects of completed Tasks are protected from deletion until Reset. Core derives protection from immutable Task result references, independently of editable Object associations. Completed Task records themselves have no delete operation, even if interfaces hide past work. [Result retention](adr/0009-expose-objects-only-when-ready.md#required-results-of-completed-tasks).
