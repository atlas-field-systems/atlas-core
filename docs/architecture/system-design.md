...
The Asset OS owns execution, interruption and its confirmed onboard queue. Core records immutable submission order and operator reorder requests separately from Asset-confirmed order. Assets execute sequentially in submission order by default, following confirmed reordering of unstarted Tasks; a disconnected Asset may continue its last confirmed order. Core records intent and outcomes without starting Tasks or gating them on connectivity. See [ADR-0007](../adr/0007-reconcile-asset-tasks-after-disconnection.md).
