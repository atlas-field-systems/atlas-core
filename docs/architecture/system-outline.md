...
| Objects | Operational information and results: metadata, content, references and provenance | Metadata lifecycle, private content storage, resumable uploads, downloads, integrity and Reset cleanup | Exposes Objects only when ready through Core APIs accessed using the SDK; hides physical storage locations and does not own each Plugin's specialized processing or result format |
...
| Runtime retention and Reset | System operations coordinates retained-state startup and Reset across stores; each module owns its state rules. Removing a Plugin does not itself reset data |
