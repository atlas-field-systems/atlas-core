...
| Identity and access | Caller identity and authenticated access | Operator identity and sessions, device/Plugin credentials, enrollment, credential revocation | Every authenticated operator has full control; no operator roles or permission tiers. Asset credentials identify the reporting Asset; Plugin credentials allow operational access; installation and Plugin lifecycle administration are local CLI/TUI responsibilities. Core enforces these fixed boundaries |
...
Use a specialized signal processor with its own Operation input controls and visualization:
