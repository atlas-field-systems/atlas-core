...
| --- | --- | --- | --- |
| Entities | Assets, Tracks and Geofeatures: identity, current state, observations, relationships and movement history within the run | Resource lifecycle, reporting/check-in, queries, movement history | Owns Entity identity and observed state, not authentication credentials or work execution |
| Tasks | Requests for Assets to execute Core-defined Commands: assignment, delivery, progress, cancellation and outcomes | Reconciliation with Asset reports and onboard queues, capability matching, runtime registration/fencing, delivery eligibility, lifecycle transitions | Core owns Command semantics and recorded Task lifecycle; the Asset operating system owns scheduling, queue order, execution and interruption. Plugins are never Task targets; Plugins may initiate Tasks through the SDK using existing Core-defined Commands |
| Objects | Current-run information and results: metadata, content, references and provenance | Metadata lifecycle, private content storage, resumable uploads, downloads, integrity and current-run cleanup | Exposes Objects only when ready through SDK/Core APIs; hides physical storage locations and does not own each Plugin's specialized processing or result format |
...
| Identity and access | Caller identity and authenticated access | Operator identity and sessions, device/Plugin credentials, enrollment, credential revocation | Every authenticated operator has full control; no operator roles or permission tiers in the first version. Credentials and operational Entities remain distinct |
...
| System operations | How Core starts empty, runs, is configured, observed and restarted | Startup/shutdown, configuration, health, diagnostics, activity history, maintenance coordination, startup reset and initialization | Does not turn every maintenance job into an operator Task or make Core its own release publisher |
...
| Runtime storage, transactions and initialization | Shared storage infrastructure clears operational stores and initializes current schemas on startup, without data migrations. Entities, Tasks, Objects and other owners define their own state rules; a generic storage module must not become a second business-rule owner |
...
| Device presence versus runtime identity | Entities represents observations such as last report; Tasks owns execution-generation fencing and delivery readiness; Identity and access owns credentials. Avoid three independent definitions of whether a device may act |
...
| External-source access policy | Keep source meaning in its integration. Shared access restrictions, rate controls or credential brokering may be a Plugin subsystem when a real integration needs them, not a mandatory separate Gateway service |
| Runtime data and restart | Every Core start wipes operational data, including content and history. System operations coordinates clean initialization across stores. No operational-data migrations, preserve-data restart or backup/restore. Installed Plugin selections, credentials and configuration survive as startup setup. Removing a Plugin without restarting Core does not by itself wipe the current run |
| Activity history | Required current-run record of Task issuance/cancellation and changes to Plugins, credentials and configuration. Proposed placement: System operations owns the shared recording/query facility; Identity and access supplies actor identity; the owning modules supply action meaning and affected resources. The external Command Interface can render it. Diagnostic logs are separate |
| System installation, updates and shared release | Core needs health, shutdown and clean initialization support. Host installation tools and the coordinated Core/SDK/Protocol publishing workflow may live elsewhere in the repository; they are not all code running inside Core |
...
Record activity consistently with the action during the current run. Core restart deliberately wipes both the data and its activity history. Store credential identifiers and safe change descriptions rather than passwords, tokens or secret configuration values. Within-run recording behavior remains to be designed; no cross-run durability, migration or recovery mechanism is required.
...
- Identity and access authenticates the operator, who has full control, and identifies the Plugin. The Plugin is trusted with operational APIs and cross-source data; credential management and system administration remain separate.
...
- Task issuance/cancellation and Plugin management changes contribute actor-attributed records to current-run activity history.
...
A single workflow cannot make separate artifact registries one transaction. Completion and recovery therefore need an explicit policy. Compatible client versions are allowed; the concrete compatibility checks, supported device upgrade lag and meaning of major/minor/patch changes remain open. Shared numbering alone answers none of those questions.

## Suggested order of design work

Design the dedicated responsibilities through concrete Atlas workflows and their public contracts. Define Protocol, SDK access and Core behavior together, with clean startup initialization and independent tests. Add shared utilities where the implemented workflows demonstrate a need. See the [system design](system-design.md) for the accepted boundaries. Command Interface implementation and individual Plugin algorithms remain outside this design.
