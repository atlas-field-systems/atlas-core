...
| Entity identity reservation | Dataset-scoped Entity ID reserved atomically on creation and retained after deletion across Restart until Reset. Reuse is rejected, including registration retries after deletion; retained history and references keep their original identity. Asset deletion atomically revokes all bound credentials and blocks concurrent provisioning for that deleted identity. |
