...
That coarse source boundary is superseded for Asset reporting. The successor authenticates each Asset, accepts execution reports only for its assigned Tasks, and prevents generic mutation paths from bypassing that rule. Plugin credentials retain operational access but cannot impersonate Assets. Plugin installation/configuration and process administration are local CLI/TUI controls, absent from the public API and SDK. See [the fixed access boundaries](../../architecture/system-design.md#identity-and-access). This does not require operator permission tiers, a tenant system or a particular authentication provider.
