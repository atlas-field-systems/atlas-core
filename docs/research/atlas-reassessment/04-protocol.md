...
| Core Go module and local Protocol replace | Core imports generated Go Protocol types and validators | [Accepted boundary] Core depends on portable Protocol contracts, never the reverse. The Go module/replace mechanism applies only if Go is selected. | Core owns routes, auth, persistence, lifecycle, and deployment; Protocol owns portable shapes and validity [boundary](https://github.com/the-Drunken-coder/Atlas-Modernization/blob/8edee4e2743fbf0f85c16dfe638d9222141cf279/docs/atlas-protocol/README.md#L19-L35). Verify no schema package imports Core or database code. |
...
| Plugin Runtime and Plugin packages | Current private operation process, SDK clients, and independent release machinery | [Proposal] Reassess the implementation and packaging while retaining the accepted Core-managed Plugin capability. | [ADR-0002](../../adr/0002-core-manages-installed-plugins.md) settles lifecycle ownership; the old exact-revision and Tool Asset machinery is not automatically inherited. |
...
| Commands, Tasks and Plugin Operations have distinct meanings | The source tasking vocabulary defines Asset Commands and their Task requests; use the [current glossary](../../../CONTEXT.md) for successor terms. | [Accepted boundary] A Command defines Asset-executable intent; a Task requests its execution; an Operation invokes a Plugin capability and may continue after caller disconnection. |
...
| Task lifecycle is explicit | Current statuses are pending, acknowledged, in progress, completed, failed, and cancelled [lifecycle](https://github.com/the-Drunken-coder/Atlas-Modernization/blob/8edee4e2743fbf0f85c16dfe638d9222141cf279/docs/atlas-protocol/commands-and-tasking.md#L296-L337). | [Accepted boundary] Represent Pending, Acknowledged, In progress, Cancellation requested, Completed, Canceled and Failed. Separate cancellation requests from Asset confirmation. Core enforces transitions and accepts completion only before confirmed Canceled, with required results ready. Exact message names remain implementation choices. |
...
Exact message names and transport remain proposals. Generate repeated contract declarations while keeping behavior in Core and SDK. Include only the authentication, Object transfer, Plugin lifecycle and synchronization support needed for that real workflow. Source Gateway and Datastream delivery are not prerequisites.
