...
Decision update: Core now [owns installed Plugin lifecycles](../../adr/0002-core-manages-installed-plugins.md) and [records activity history only within a Core run](../../adr/0013-start-each-core-run-with-empty-data.md). Earlier alternatives in this research remain evidence for implementation tradeoffs, not unresolved choices about whether Core manages Plugins or records these actions.
...
| D4 | Metadata and content storage | PostgreSQL baseline; compare fewer-service storage | PostgreSQL plus local content; SQLite plus local content; bounded bytes in DB if workload permits | Expected write/byte workload, clean restart across stores, concurrent reads, disk-full handling and operator effort |
...
| D7 | Operator and device authority | All authenticated operators have full control; retain caller identity for activity history. Operator roles are excluded from the first version | Evaluate authentication and device/Plugin identity mechanisms without introducing operator permission tiers | Define enrollment, credential rotation/revocation, device access boundaries and trusted Plugin operational access and current-run action attribution |
| D8 | Deployment and update product | One documented server package and one primary operator path | Simple Compose; binary/service manager; interactive manager when its UX is required | Fresh-host installation, clean restart, update startup and credential rotation all have tested outcomes |
| D9 | Runtime history and reset | Every Core start wipes operational data; preserve observed-time semantics only within the run | Storage implementations that provide coherent in-run behavior and clean initialization | Verify no previous-run metadata, bytes, activity history or client replay silently survives restart; no migration or backup/restore |
...
### E3. Runtime storage and clean restart comparison
...
Pass when every Core start initializes empty operational stores, rollback creates no published mutation within a run, startup setup survives and interrupted uploads cannot expose unusable Objects. Record p50/p95/p99 request latency, sustained write rate, lock waits, per-run storage growth and clean-start duration. Set the acceptance thresholds from the product workload before ranking candidates.
