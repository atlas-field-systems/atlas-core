...
| D9 | Runtime history and lifecycle | Start, Stop and Restart preserve data/logs; Reset clears them while preserving setup and artifacts | Storage and log implementations that satisfy the lifecycle | Verify both retention and clearing across metadata, content, history, transfer/sync state and logs; updates to new Core releases perform Reset; operational-data migrations are excluded and execution resumption is a separate policy |
