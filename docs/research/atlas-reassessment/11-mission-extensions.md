...
3. Block planned stops and updates until active Plugin work finishes or the operator explicitly cancels it. Record an honest outcome. Keep the outcomes of Asset Tasks that produced input Objects separate from the processing outcome.
...
5. Verify that current-run results and provenance remain readable after Plugin removal. Separately restart Core and verify that operational data and logs remain available, then Reset and verify that they are cleared while installation setup survives. Keep extension identity, version, relevant settings and input references when reproducibility requires them.
