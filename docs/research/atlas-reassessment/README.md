...
Current design update: [ADR-0014](../../adr/0014-build-dedicated-atlas-systems.md) selects dedicated Atlas systems over an infrastructure-first framework. The [system design](../../architecture/system-design.md) records SDK modes, private Object storage, Asset-owned scheduling and client-independent Plugin Operations. Core normally stays running while Assets operate. Start, Stop and Restart retain records and logs; Reset clears them. Active execution resumption is a separate contract. Earlier research proposals do not override these decisions.
