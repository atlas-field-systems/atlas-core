...
In full-synchronization or Asset hybrid mode, route successful in-scope write responses through the same ordered reconciliation path as feed/recovery changes. Buffer the response until the synchronizer establishes that all preceding relevant changes and all effects of the write's commit have been applied. Before that boundary, do not expose the response as picture state, emit its local feed notification, append it to local change history or resolve the SDK write as synchronized success. The response or its correlated feed record must identify the Dataset and commit boundary; exact fields remain Protocol work.

For example, if the picture has applied N and a write response for N+2 arrives before N+1, apply N+1 first, then N+2. Emit and retain each change once, deduplicating the corresponding response/feed delivery in either arrival order. A single-resource response cannot stand in for other resources changed by the same commit. Version checks prevent an older response from replacing newer already-applied state or resurrecting a deleted resource. A mutation response alone never advances the global recovery cursor. Hybrid recovery uses Core's scoped continuation/barrier proof for excluded changes; it must not download out-of-scope resources or wait for every integer sequence to appear.

Resolve the write after the picture has reached its commit boundary or a later consistent boundary, even if a later update or deletion already supersedes its resource value. If replay has expired, a fresh consistent snapshot at or beyond that boundary can satisfy reconciliation; invalidate the old local cursor generation and report the rebuild rather than inventing missing historical events. If synchronization cannot reach the boundary before a timeout or disconnection ends the wait, return an explicit committed-but-not-yet-synchronized outcome carrying the known Core result. Keep the picture's stale/not-ready status accurate and do not automatically resubmit the mutation. A Dataset change instead follows the Reset rule below and cannot insert the old result into the new picture. Exact outcome types and wait limits remain SDK contract details.
