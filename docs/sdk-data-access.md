...
In full-synchronization or Asset hybrid mode, after Core confirms a successful write affecting the mode's operational picture, reconcile its authoritative result into local state before resolving the SDK write. Emit each applied change once, and deduplicate the matching feed event. Resource-version checks prevent an older write response from replacing newer state or resurrecting a deleted resource. This must not skip other events by advancing the global recovery cursor to the response's version. Failed writes never appear as committed cache updates. HTTP mode continues to return Core's response without maintaining a local picture.
