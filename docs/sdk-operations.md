...
Full-synchronization mode maintains the full operational dataset in memory, with no persistence or selective synchronization. It rebuilds on SDK restart and reports resource-limit failures rather than silently dropping resources. Successful in-scope writes pass through [ordered reconciliation](sdk-data-access.md#writes) before resolving as synchronized success. Buffer responses until preceding relevant changes and the write's commit effects are applied; then expose local state, feed and history consistently, without duplicate response/feed events or regression. A confirmed Core commit whose synchronization wait fails has an explicit committed-but-not-yet-synchronized outcome and is not automatically resubmitted. Writes rejected by Core do not appear locally as committed data.
