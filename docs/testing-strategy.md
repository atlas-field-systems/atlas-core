...
| Asset identity and contact | Deployment-authorized automatic enrollment without per-Asset approval, enrollment retry, two distinct Assets, valid own reports and rejected cross-Asset reports across every mutation path; fresh reports versus duplicates/stale reports; administrative boundaries and revocation, including already-open feeds, buffered-event races and reconnect rejection. |
...
| Reset and Restart | Old responses, submissions, retries and cursors rejected after Reset in every mode; no obsolete state resurrection; retained Dataset ID/writing release, Task creation retry identities and change window after Restart; refusal on writing-release mismatch; interrupted Operations without automatic rerun. |
| Objects and histories | Interrupted full-file upload and cleanup, lost success response, concurrent identical retries, conflicting identity reuse, ready-only visibility, immutable bytes, download equality, SDK-allocated result IDs with report-before-upload and upload-before-report, conflicting ID claims, deleted-upload retries without resurrection, protected required results of completed Tasks, deletion conflicts without optimistic SDK removal, metadata-association edits that cannot release protection, shared required Objects, completion/deletion races and Reset cleanup, movement deduplication and stable history pagination, activity attribution surviving profile/credential deletion without secrets. |
